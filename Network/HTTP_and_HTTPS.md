...
> __대칭키(=Session key)는 어떻게 만들어 지는가?__ </br>
> 클라이언트는 전달받은 서버의 랜덤 데이터와 본인이 생성한 랜덤 데이터를 조합해서 pre master secret를 생성 </br>
>  → 생성한 pre master secret는 공개키 방식으로 암호화 해서 서버로 전송</br>
>  → 서버와 클라이언트는 모두 일련의 과정을 거쳐 pre master secret 값을 master secret 값으로 만듦 </br>
>  → Master secret는 session key를 생성 </br>
>  이 session key 값을 이용해 서버와 클라이언트는 데이터를 대칭키 방식으로 암호화하여 통신한다. </br>
