...
### 가상 메모리 스레싱(Thrashing)

페이지 부재율이 너무 높아 작업이 거의 이루어지지 않는 상태

활발하게 사용되는 페이지를 충분히 메모리에 올리지 못하고 스왑 영역으로 접근이 많아져 느려지는 현상

#### 스레싱의 원인

멀티프로그래밍 정도(동시에 실행하는 프로그램의 수)가 물리 메모리에 비해 너무 높으면 스레싱이 발생한다.

![Thrashing](./img/VM_DP_PRA0.png)

프로그램 수가 적을 때는 CPU 사용률이 계속 증가하다가 메모리가 꽉 차면 CPU가 작업하는 시간보다 스왑 영역에 접근하는 작업이 빈번해져서 CPU가 작업할 수 없는 상태에 이른다. 이러한 시점을 스레싱 발생 시점이라고 한다.

#### 스레싱 해결 방법

- 물리 메모리 용량을 늘려 스레싱 발생 지점을 늦춤
- 페이지 부재 빈도를 기록하며 페이지의 양을 동적으로 조정하여 각 프로세스가 필요로 하는 최소한의 프레임 갯수를 보장해줌
- 지역 교환 알고리즘(교체 알고리즘을 현재 실행 중인 프로세스의 프레임에만 적용)
- 우선 순위 교체 알고리즘
...
> Reference
> - [https://faithpac27.tistory.com/entry/쓰레싱-Thrashing-이란](https://faithpac27.tistory.com/entry/%EC%93%B0%EB%A0%88%EC%8B%B1-Thrashing-%EC%9D%B4%EB%9E%80)
> - [https://blog.naver.com/jevida/140192459532](https://blog.naver.com/jevida/140192459532)